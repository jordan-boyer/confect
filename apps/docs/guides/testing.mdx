...
<Note>
  With **Effect 4**, schema-driven calls surface failures as **`Schema.SchemaError`** (not the older `ParseResult.ParseError` type). `TestConfect.run(handler, returns)` types the error channel as **`E | Schema.SchemaError`** when you pass a returns schema, where `E` is the handler’s error type.
</Note>
...
`TestConfect` provides `query`, `mutation`, and `action` methods that accept a ref and args, returning an `Effect` that succeeds with the decoded result or fails with **`Schema.SchemaError`** if encoding args or decoding the return value fails.
