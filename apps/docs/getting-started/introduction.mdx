...
Confect is a framework that deeply integrates [Effect](https://effect.website) with [Convex](https://convex.dev). It targets **Effect 4** (`effect` 4.x). It's more than just Effect bindings! Confect allows you to:
...
- Use Effect's HTTP API modules (`effect/unstable/httpapi`) to define your HTTP API(s). Includes interactive OpenAPI documentation powered by [Scalar](https://github.com/scalar/scalar).
