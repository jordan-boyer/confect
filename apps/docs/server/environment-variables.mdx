...
In Effect 4, `Config.string(...)` returns **`Config<T>`**, not **`Effect`**. You either **`yield*`** it inside `Effect.gen`, or wrap it with **`Effect.fromYieldable`** to get an `Effect` (same runtime behavior).
...
const getApiKey = FunctionImpl.make(
  ...
  "getApiKey",
  () =>
    Effect.fromYieldable(Config.string("API_KEY")).pipe(Effect.orDie),
);
```
...
const getApiKey = FunctionImpl.make(
  ...
  "getApiKey",
  () =>
    Effect.fromYieldable(Config.string("API_KEY")).pipe(
